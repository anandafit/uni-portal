...
class Uniportal
{
	...
	public function __construct()
	{
		...
		$this->ci->load->config('uniportal', TRUE);
		$this->ci->load->library('email');
		$this->ci->load->library('session');
		$this->ci->lang->load('uniportal');
...
//		$this->error_end_delimiter     = $this->ci->config->item('error_end_delimiter', 'ion_auth');
		...
		{
			$this->ci->auth = $this;
			$this->ci->users_model->login_remembered_user();
		}
	}
	...
	{
		return (bool) $this->ci->session->userdata(APP_NAME.'_access');
	}
	...
	{
    return _isLogin($this->ci->config->item('ADMIN_GROUP', 'uniportal'));
	}

  /** check for is student role
   * @return mixed
   */
  public function isStuent(){
    return _isLogin($this->ci->config->item('STUDNET_GROUP', 'uniportal'));
  }

  /** check for is teacher
   * @return mixed
   */
  public function isTeacher(){
    return _isLogin($this->ci->config->item('TEACHER_GROUP', 'uniportal'));
  }
   ...
   * @return bool
   */
  private function  _isLogin( $group){
    if($this->ci->session->userdata(APP_NAME.'_access') && ($this->ci->session->userdata("LAST_ACTIVETY_AT") + $this->ci->config->item('TEACHER_GROUP', 'USER_TIME_OUT') >= time())){
